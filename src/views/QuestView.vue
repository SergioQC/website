...
const SECRET_CODE = '040925' // change this to the code you want
const codeInput = ref('')
const codeError = ref('')
          ...
          <div class="row-squares">
            ...
            <div class="square check-square">
              ...
              <div v-if="showDialog" class="overlay">
                <div class="modal">
                  <button class="modal-close" @click="closeDialog" aria-label="Close">×</button>
                  <h2>Congratulations!</h2>
                  <p>You've completed all the words! GQ!</p>
...
.modal p { margin: 0 0 1rem 0; color: var(--sl-color-neutral-700); }

.modal-actions { display:flex; gap:0.5rem; justify-content:center }
.modal .btn { padding: 0.5rem 0.75rem; border-radius:6px; border: none; cursor: pointer }
.modal .btn.reset { background: #f3f4f6; color: #111827 }
.modal .btn.close { background: var(--sl-color-primary-600); color: white }

.code-label { display:block; font-size:0.9rem; margin-bottom:0.5rem; color:var(--sl-color-neutral-700) }
.code-row { display:flex; gap:0.5rem; align-items:center; justify-content:center; margin-bottom:0.5rem }
.code-input {
  width:160px;
...
.modal .btn.submit { background: var(--sl-color-primary-600); color:white }
.code-msg { min-height:1.25rem }
.code-msg .error { color: #dc2626 }
.code-msg .success { color: #16a34a }
.final-word { margin-top:0.5rem; color:var(--sl-color-neutral-900) }
...
.square.highlighted .index { color: var(--sl-color-primary-600) }
...
.modal-close:hover { background: rgba(0,0,0,0.06); color: var(--sl-color-neutral-800) }
</style>